...
use jandering_engine::{
    ...
    window::{Key, WindowConfig, WindowEvent, WindowManagerTrait, WindowTrait},
};
...
fn main() {
    ...
    let mut render_data = RenderDataBindGroup::new(renderer);
    ...
    let heightmap_texture = TextureSamplerBindGroup::new(renderer, heightmap_handle, tex_sampler);
    ...
    let noise_texture = TextureSamplerBindGroup::new(renderer, noise_handle, tex_sampler);
    ...
    let terrain_shader_code = include_str!("terrain_shader.wgsl").to_string();

    let grass_shader = renderer.create_shader(ShaderDescriptor {
        name: "terrain shader",
        source: ShaderSource::Code(terrain_shader_code.clone()),
    ...
    let ground_shader = renderer.create_shader(ShaderDescriptor {
        name: "ground shader",
        source: ShaderSource::Code(terrain_shader_code),
    ...
    engine.run(move |renderer, window_manager| {
        ...
        } else if events.is_mouse_pressed(jandering_engine::window::MouseButton::Left) {
            is_in_fps = true;
