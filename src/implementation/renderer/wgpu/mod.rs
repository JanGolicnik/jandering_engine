...
pub(crate) struct WGPUComputeShader {
...
impl Janderer for WGPURenderer {
    ...
    fn submit_pass(&mut self, pass: RenderPass) {
        ...
        for (i, step) in steps.iter().enumerate().take(len) {
            ...
            for (index, handle) in bind_groups.iter() {
                render_pass.set_bind_group(*index, &self.bind_groups[handle.0], &[]);
            }
    ...
    fn create_bind_group(&mut self, layout: BindGroupLayout) -> BindGroupHandle {
        let handle = BindGroupHandle(self.bind_groups.len());
        self.create_bind_group_at(layout, handle);
        handle
    }
