...
use super::{Events, FpsPreference, WindowConfig, WindowId, WindowResolution, WindowTrait};
...
impl WindowTrait for WinitWindow {
    ...
    fn set_fps_prefrence(&mut self, preference: super::FpsPreference) {
        let mut window = self.inner_window.lock().unwrap();
        match &mut *window {
    ...
    fn get_fps_prefrence(&self) -> super::FpsPreference {
        let window = self.inner_window.lock().unwrap();
        match &*window {
...
impl InnerWinitWindow {
    pub(crate) fn handle_event(&mut self, event: super::WindowEvent) {
        ...
        match event {
            super::WindowEvent::RedrawRequested => {
                if let super::FpsPreference::Exact(fps) = fps_preference {
                    #[cfg(target_arch = "wasm32")]
                    panic!();
            ...
            super::WindowEvent::CloseRequested => *should_close = true,
            _ => events.push(event),
        }
    }
}
