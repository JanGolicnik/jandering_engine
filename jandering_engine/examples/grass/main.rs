...
use jandering_engine::{bind_group::{camera::free::{FreeCameraController, MatrixCameraBindGroup}, texture::TextureBindGroup, BindGroup, BindGroupLayout, BindGroupLayoutEntry}, engine::Engine, object::{Instance, Object, Vertex}, renderer::{BufferHandle, Janderer, Renderer}, shader::{ShaderDescriptor, ShaderSource}, texture::{sampler::SamplerDescriptor, texture_usage, TextureDescriptor, TextureFormat}, types::{UVec2, Vec3}};
use je_windowing::{InputState, Key, WindowConfig, WindowEvent, WindowManagerTrait, WindowTrait};
...
fn main() {
    ...
    let render_data_bind_group_layout = render_data.get_layout();
    ...
    let heightmap_texture =
        TextureBindGroup::new(renderer, heightmap_handle, tex_sampler);
    ...
    let noise_texture =
        TextureBindGroup::new(renderer, noise_handle, tex_sampler);
    ...
    let grass_shader = renderer.create_shader(
        ShaderDescriptor::default()
            ...
            .with_bind_group_layouts(vec![
                ...
                heightmap_texture.get_layout(),
                noise_texture.get_layout(),
            ])
            .with_depth(true)
    ...
    engine.run(move |renderer, window_manager|{
        ...
        if fps_accumulator > 1.0 {
            println!(
                "fps: {}",
                1.0 / (fps_accumulator / fps_counter as f32)
            );
            fps_accumulator = 0.0;
            fps_counter = 0;
        }
{
        let render_data = renderer.get_typed_bind_group_mut(render_data_handle).unwrap();
            ...
            .any(|e| matches!(e, WindowEvent::Resized(_)))
        {
            ...
            let camera = renderer.get_typed_bind_group_mut(camera_handle).unwrap();
            camera.make_perspective(CAMERA_FOV, size.x as f32 / size.y as f32, CAMEREA_NEAR, CAMEREA_FAR);
        ...
        if window.is_initialized() {
            let camera = renderer.get_typed_bind_group(camera_handle).unwrap();
            renderer.write_bind_group(camera_handle.into(), &camera.get_data());
    
            let render_data = renderer.get_typed_bind_group_mut(render_data_handle).unwrap();
            let sky_color = render_data
                .data
                .sky_color;
            ...
            renderer.write_bind_group(render_data_handle.into(), &render_data.get_data());

            let mut pass = renderer.new_pass(&mut window);
            pass
            .set_shader(star_shader)
            .with_target_texture_resolve(multisample_texture, None)
